...
handler = logging.FileHandler('excel2json_log.txt')
...
def parse_excel_list(sh, info, start):
    ...
    for c in range(0, sh.ncols):
        fvalue = sh.cell(0+start, c).value
        if not fvalue:
            continue
...
def parse_excel_mat(sh, info, start):
    ...
    col_head = [convert(sh.cell(start, i).value, pyt)
                for i in range(start + 1, sh.ncols)]
    row_head = [convert(sh.cell(i, start).value, pyt)
...
def field_matching(info_dict, match):
    """字段匹配
                ...
                for item in info.data:
                    ...
                    for t in target.data:
                        ...
                        if found:
                            ...
                            logger.error("forign not found:{0} -> {1} {2}:{3}".format(info.name, field_name, attrs, conds))
                    item[field_name] = query_result


def merge_plus_tables(info_dict):
    """合并带+号的数据表格。
    ...
    Args:
        info_dict
    """
    for info in list(info_dict.values()):
        if '+' in info.filename:
            del info_dict[info.name]
            arr = os.path.splitext(info.filename)[0].split('+')
            main_table = info_dict[arr[0]]
            if not main_table:
                logger.error("main table not found:{0}".format(info.filename))
            else:
                pk = main_table.fields[0].name
                if main_table.type == TYPE_LIST or main_table.type == TYPE_DICT and pk == info.fields[0].name:
                    for sub in info.data:
                        if not pk in sub:
                            continue
                        pk_value = sub[pk]
                        for main in main_table.data:
                            if pk_value == main[pk]:
                                for k in sub:
                                    main[k] = sub[k]
                                break
                ...
                for i in range(1, len(info.fields)):
                    main_table.fields.append(info.fields[i])


def get_data_dict(info_dict):
    """生成导出数据字典
    ...
    for info in info_dict.values():
        if info.type == TYPE_DICT:
            ...
            data = info.data
        ret_dict[info.name] = data
    return ret_dict


def merge_array_element_fields(info_dict):
...
def build_simple_array_table(info_dict):
    """构造简单的数组
...
def get_meta_dict(info_dict):
    ...
    for info in info_dict.values():
        ...
        if info.type == TYPE_DICT:
            ...
            pk = ''
        meta_dict[info.name] = {"type": info.type,
                           "primary_key": pk, "fields": fields}
    return meta_dict


def diff_meta(last_meta_file, meta):
    ...
    Returns:
        ...
        False meta信息没有变更
    """
    if os.path.isfile(last_meta_file):
        with open(last_meta_file, encoding='utf-8') as f:
            last_def = json.load(f)
            return not last_def == meta
    return False


def get_excel_info_dict(src, excludes, start=0):
...
def parse(src, match, excludes, start=0):
    info_dict = get_excel_info_dict(src, excludes, start)

    assemble_foreign_item(info_dict)

    if match:
        field_matching(info_dict, match)

    merge_plus_tables(info_dict)
    merge_array_element_fields(info_dict)
    build_simple_array_table(info_dict)

    ret_data_dict = get_data_dict(info_dict)
    ret_meta_dict = get_meta_dict(info_dict)
...
def main():
    ...
    args.add_argument('--excludes', default='', help="排除文件")
    args.add_argument('--workspace', default=None, help="设置工作区目录")
    arg = args.parse_args()

    if arg.workspace:
        os.chdir(arg.workspace)

    assert arg.meta and arg.file and arg.excel

    excludes = arg.excludes.split(',')
    # dic, msg = parse_excels(arg.excel, arg.match, excludes, arg.start)
    dic, msg = parse(arg.excel, arg.match, excludes, arg.start)

    if diff_meta(arg.meta, msg):
        logger.error("两次数据导入定义发生改变，请检查并修正。如确认需要变更，请删掉之前导出的数据历史，从新再次导出即可")
    else:
        # 定义没有变更，导出meta文件
